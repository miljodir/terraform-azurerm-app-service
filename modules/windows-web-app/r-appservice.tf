resource "azurerm_windows_web_app" "app_service_windows" {
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      minimum_tls_version               = lookup(site_config.value, "min_tls_version", !startswith(local.app_service_name, "p-") ? "1.2" : "1.3")
      ...
      scm_minimum_tls_version     = lookup(site_config.value, "scm_minimum_tls_version", !startswith(local.app_service_name, "p-") ? "1.2" : "1.3")
      ...
      dynamic "application_stack" {
        ...
        content {
          ...
          python         = lookup(local.site_config.application_stack, "python", null)
        }
      }
...
resource "azurerm_role_assignment" "kv_secrets_user" {
  ...
  principal_id         = azurerm_windows_web_app.app_service_windows.identity[0].principal_id
